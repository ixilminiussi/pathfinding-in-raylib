...
float Soldier::separationRangeSqr =
    (Soldier::radius + 9.0f) * (Soldier::radius + 9.0f);
...
Soldier::Soldier(const Vector2 &position)
    : position(position), direction(), isSelected(false), isTravelling(false),
      path(nullptr), objective({0.0f, 0.0f}), lastTimeImmobile(0.0f) {
    army.push_back(this);
}
...
void Soldier::deselect() {
    isSelected = false;
    selected.erase(std::remove(selected.begin(), selected.end(), this),
                   selected.end());
}

const Vector2 &Soldier::getPosition() const { return position; }
...
void Soldier::update(float dt) {
    ...
    // follow path
    if ((path != nullptr) && isTravelling &&
        (Vector2DistanceSqr(objective, position) > 4000.0f)) {
        V = path->getDirectionFromNearby(position);

        impulse = Vector2Add(impulse,
                             Vector2Scale(Vector2Subtract(V.origin, position),
                                          towardsPathStrength));
        impulse = Vector2Add(impulse,
                             Vector2Scale(V.direction, alongsidePathStrength));
        ...
        if (path != nullptr) {
            if (Vector2DistanceSqr(objective, position) > 10.0f) {
                impulse = Vector2Add(
                    impulse, Vector2Scale(Vector2Subtract(objective, position),
                                          alongsidePathStrength));
            }
        }
    }
    ...
    for (Soldier *s : army) {
        if (!((!isTravelling || !s->isTravelling) && s->unitID == unitID) ||
            (s->isTravelling && s->unitID != unitID) ||
            (isTravelling && s->isTravelling)) {
            if (Vector2DistanceSqr(position, s->position) <=
                separationRangeSqr) {
                impulse = Vector2Add(
                    impulse,
                    Vector2Scale(Vector2Subtract(position, s->position),
                                 separationStrength));
                if (isTravelling) {
                    impulse = Vector2Add(
                        impulse, Vector2Scale(V.direction,
                                              2.0f * alongsidePathStrength));
                }
            }
        }
    }
    ...
    Vector2 worldIndexMin = world->getWorldAddress(
        Vector2Subtract(position, Vector2{obstacleRange, obstacleRange}));
    Vector2 worldIndexMax = world->getWorldAddress(
        Vector2Add(position, Vector2{obstacleRange, obstacleRange}));

    // iterate only through nearby walls
    for (int x = (int)std::floor(worldIndexMin.x);
         x < (int)std::ceil(worldIndexMax.x); x++) {
        for (int y = (int)std::floor(worldIndexMin.y);
             y < (int)std::ceil(worldIndexMax.y); y++) {
            if (world->getTileCategory(x, y) == TileCategory::WALL) {
                Rectangle rectangle = world->getRectangle(x, y);
                Vector2 rectangleCenter = {rectangle.x + rectangle.width / 2.0f,
                                           rectangle.y +
    ...
    if (path != nullptr) {
        if (!Vector2Equals(impulse, Vector2Zero())) {
            ...
            if (GetTime() - lastTimeImmobile >
                1.5) { // after being immobile for
