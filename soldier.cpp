...
void Soldier::update(float dt)
{
        ...
        for (int y = (int)std::floor(worldIndexMin.y); y < (int)std::ceil(worldIndexMax.y); y++) {
            if (world->getType(x, y) == Type::WALL) {
				...
				if (CheckCollisionCircleRec(position, obstacleRange, rectangle)) {
					impulse = Vector2Add(impulse, Vector2Scale(Vector2Subtract(position, rectangleCenter), obstacleStrength));
				}
            }
        }
    }
    ...
    if (!Vector2Equals(impulse, Vector2Zero())) {
        ...
        if (GetTime() - lastTimeImmobile > 2.0) { // after being immobile for long enough, lose your path
            path = nullptr;
        }
    }
